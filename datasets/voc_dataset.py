...
class VOCDataset(data.Dataset):
    ...
    def __init__(self,
        ...
        image_set_abs = os.path.abspath(self.image_set)
        main_dir = os.path.dirname(image_set_abs)  # .../ImageSets/Main
        imagesets = os.path.dirname(main_dir)  # .../ImageSets
        self.data_path = os.path.dirname(imagesets)  # 数据集根目录（含 JPEGImages/Annotations
    ...
    def _image_path_from_index(self, index):
        for ext in self.image_ext:
            image_path = os.path.join(self.data_path, 'JPEGImages', index + ext)
            if os.path.exists(image_path):
                return image_path
        raise FileNotFoundError(
            f'Image not found for id "{index}" under '
            f'{os.path.join(self.data_path, "JPEGImages")} with extensions {self.image_ext}'
        )
